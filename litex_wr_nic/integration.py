...
ERR_WR_CORES_STALE_TREE   += "This usually means an older WR-cores checkout is present in the build directory. "
...
def _normalize_wr_nic_dir(wr_nic_dir):
    if wr_nic_dir is None:
        return None

    build_script = os.path.join(wr_nic_dir, WR_FIRMWARE_BUILD_SCRIPT_REL)
    if os.path.isfile(build_script):
        return wr_nic_dir

    nested = os.path.join(wr_nic_dir, "litex_wr_nic")
    nested_build_script = os.path.join(nested, WR_FIRMWARE_BUILD_SCRIPT_REL)
    if os.path.isfile(nested_build_script):
        return nested
