...
def phy8_sources(platform):
    ...
    from litex_wr_nic.gateware.wr_common import (
        wr_core_init, wr_core_files, patch_wr_subsystem_mux_class,
    ...
    for filename in wr_core_files:
        ...
        filename = filename.replace('genrams/xilinx/gc_shiftreg.vhd',
            'genrams/altera/gc_shiftreg.vhd')
    ...
    for filename in (
        ...
        'wr_nmea/nmea_master_regs.vhd', 'wr_nmea/wr_nmea_master.vhd', 'wr_nmea/xwr_nmea_master.vhd',
        'wr_auxclk_gen/auxclk_regs.vhd', 'wr_auxclk_gen/xwr_auxclk_gen.vhd',
    ):
        sources.append(os.path.abspath('wr-cores/modules/' + filename))
    sources.append(os.path.join(os.path.dirname(__file__),
        'wr-cores/board/litex_wr_nic/xwrc_litex_phy8.vhd'))
    return sources
...
class GW5WRPHY(LiteXModule):
    ...
    def __init__(self, platform, lane=0):
        ...
        self.comb += [encoder.d[0].eq(self.tx_data), encoder.k[0].eq(self.tx_k)]
        self.sync.wr_phy_tx += [
        ...
        self.status = CSRStatus(5, description='TX PLL lock, RX CDR lock, aligned, RX valid, ready.')
        self.specials += MultiReg(Cat(serdes.pll_lock, serdes.cdr_lock,
            serdes.aligned, serdes.rx_valid, self.ready), self.status.status)
