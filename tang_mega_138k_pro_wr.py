...
class BaseSoC(SoCMini):
    def __init__(self, sfp=0, with_analyzer=False, analyzer_csv='analyzer.csv',
        ...
        self.uart_xover = UART(self.uart_xover_phy,
            tx_fifo_depth=128, rx_fifo_depth=128, rx_fifo_rx_we=True)
        wr_serial = Record([('tx', 1), ('rx', 1)])
        self.comb += [wr_serial.rx.eq(console_pads.tx), console_pads.rx.eq(wr_serial.tx)]
        ...
        self.add_ram('wr_cpu_ram', origin=0x1000_0000, size=128*1024,
            contents=get_mem_data(str(Path(cpu_firmware).with_suffix('.bin')),
                endianness='little', mem_size=128*1024))
        ...
        self.comb += [wr.source.ready.eq(1), wr.sink.valid.eq(0)]
        ...
        if with_analyzer:
            ...
            }, depth=1024, samplerate=125e6, clock_domain='wr_phy_rx',
                trigger_depth=128, subsampler_width=4, csr_csv=analyzer_csv)
        self.comb += [
...
def main():
    ...
    parser.add_argument('--sfp', type=int, choices=(0, 1), default=0)
    parser.add_argument('--with-analyzer', action='store_true', help='Capture raw and decoded RX symbols with LiteScope.')
    parser.add_argument('--skip-firmware-build', action='store_true')
    parser.add_argument('--output-dir', default='build/tang_mega_138k_pro_wr')
    args = parser.parse_args()
    if not args.skip_firmware_build:
        subprocess.run([sys.executable, 'litex_wr_nic/firmware/build.py',
            '--target', 'tang_mega_138k_pro', '--read-only-storage'], check=True)
    soc = BaseSoC(sfp=args.sfp, with_analyzer=args.with_analyzer,
